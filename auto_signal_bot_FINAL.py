...
def get_btc_regime() -> str:
    """Return the BTC macro regime: 'uptrend', 'downtrend', 'neutral', or 'ranging'.

    Uses four complementary lenses:
      1. EMA trend (daily + 4H)   — directional structure
    ...
    ranging  → ADX < 22  AND  Donchian=inside  AND  DFT trend-pct < 0.45
               (or both daily + 4H neutral and EMAs tight)
    uptrend  → daily + 4H EMA both uptrend; ADX ≥ 20
    downtrend→ daily OR 4H EMA downtrend; ADX ≥ 20
    neutral  → everything else (transitional)
    """
    global _btc_1h_cache
    try:
        candles_daily = fetch_klines("BTCUSDT", "1d", 220)
        ...
        daily_trend = detect_trend(candles_daily) if len(candles_daily) >= 200 else "neutral"
        h4_trend    = detect_trend(candles_4h)    if len(candles_4h)    >= 50  else "neutral"

        closes_4h = [c["close"] for c in candles_4h]
        e20_4h    = ema(closes_4h, 20)
        e50_4h    = ema(closes_4h, 50)
        spread_pct = (abs(e20_4h - e50_4h) / closes_4h[-1]) \
                     if (e20_4h and e50_4h and closes_4h) else 0.05
        ...
        adx_str = f"{adx_val}" if adx_val else "n/a"
        print(f"[BTC Regime] daily={daily_trend} | 4H={h4_trend} | "
              f"spread={spread_pct:.1%} | ADX={adx_str} | "
        ...
        if adx_ranging and don_state == "inside" and dft_ranging:
            regime = "ranging"
        elif daily_trend == "neutral" and h4_trend == "neutral" and spread_pct < 0.025:
            regime = "ranging"

        # DOWNTREND: at least one TF confirms down, trend is strong enough
        elif (daily_trend == "downtrend" or h4_trend == "downtrend"):
            regime = "downtrend"

        # UPTREND: both TFs up, trend has strength
        elif daily_trend == "uptrend" and h4_trend == "uptrend":
            regime = "uptrend"

        # NEUTRAL: transitional (one TF still neutral)
        else:
            regime = "neutral"
...
def calc_macd(candles, fast=12, slow=26, sig_period=9):
    """Returns (macd_line, signal_line, histogram) or (None,None,None)."""
    closes = [c["close"] for c in candles]
    if len(closes) < slow + sig_period + 5:
        return None, None, None
    # Build fast EMA series
    k_fast = 2 / (fast + 1)
    fe = [sum(closes[:fast]) / fast]
    for p in closes[fast:]:
        fe.append(p * k_fast + fe[-1] * (1 - k_fast))
    # Build slow EMA series
    k_slow = 2 / (slow + 1)
    se = [sum(closes[:slow]) / slow]
    for p in closes[slow:]:
        se.append(p * k_slow + se[-1] * (1 - k_slow))
    # MACD line (aligned)
    offset = slow - fast
    macd_line = [fe[i + offset] - se[i] for i in range(len(se))]
    if len(macd_line) < sig_period:
        return None, None, None
    # Signal line
    k_sig = 2 / (sig_period + 1)
    sig = sum(macd_line[:sig_period]) / sig_period
    for m in macd_line[sig_period:]:
...
def analyse_token(symbol, token_data):
    ...
    # MACD (1H) — momentum confirmation
    macd_val, macd_sig, macd_hist = calc_macd(candles_1h)
    macd_bullish = macd_hist is not None and macd_hist > 0 and macd_val is not None and macd_val > macd_sig
    macd_bearish = macd_hist is not None and macd_hist < 0 and macd_val is not None and macd_val < macd_sig
    ...
    # ── TERTIARY: MACD (confirmation only, low weight) ────────────────
    if (direction == "LONG" and macd_bullish) or (direction == "SHORT" and macd_bearish):
        confluences.append(f"MACD aligned ({'↑' if direction=='LONG' else '↓'})")
        score += 6
    elif (direction == "LONG" and macd_bearish) or (direction == "SHORT" and macd_bullish):
        confluences.append("MACD opposing — conviction reduced")
        score -= 4
...
def deepseek_score_and_summarise(analysis, eth_res, bsc_res, sol_res,
    ...
    macd_str     = ("bullish" if analysis.get("macd_bullish") else
                    "bearish" if analysis.get("macd_bearish") else "flat/neutral")
...
MACD(1H): {macd_str}  RSI Divergence: {rsi_div_str}
...
def fire_signal(analysis, eth_res, bsc_res, sol_res, onchain_combined,
    ...
    msg = (
        f"{emoji} <b>AUTO SIGNAL #{signal_id}</b>{flip_banner}\n"
        f"{'─'*32}\n"
        f"🪙 <b>{sym} — {dire}</b>\n"
        f"🏅 Grade: <b>{gs['label']}</b>\n"
        f"📈 Est. Win Rate: <b>{gs['win_pct']}%</b>  |  "
        f"AI Conviction: <b>{gemini_score}/100</b>\n"
        f"💵 Est. P&amp;L → TP1: <b>{gs['avg_pnl_tp1']}</b>  |  TP2: <b>{gs['avg_pnl_tp2']}</b>\n"
        f"{'─'*32}\n"
        f"📍 Entry:     <b>{analysis['entry']}</b>\n"
        f"🛑 Stop Loss: <b>{analysis['sl']}</b>  ({'+'if dire=='SHORT' else '-'}{sl_pct}%)\n"
        f"🎯 TP1 (1R):  <b>{analysis['tp1']}</b>  ({'-'if dire=='SHORT' else '+'}{tp1_pct}%)\n"
        f"🎯 TP2 (2R):  <b>{analysis['tp2']}</b>  ({'-'if dire=='SHORT' else '+'}{tp2_pct}%)\n"
        f"{'─'*32}\n"
        ...
        f"MACD: {'🟢 Bull' if analysis.get('macd_bullish') else ('🔴 Bear' if analysis.get('macd_bearish') else '⚪️ Flat')} | "
...
def run_scanner():
    ...
    # Fire a Telegram notification the moment BTC breaks out of a
    # choppy/transitional state into a clear trend — so you know
    # immediately when the bot is "unlocked" and scanning for real signals.
    _was_blocked   = _prev_btc_regime in ("ranging", "neutral", "")
    _now_trending  = btc_regime in ("uptrend", "downtrend")
    _was_trending  = _prev_btc_regime in ("uptrend", "downtrend")
    _now_blocked   = btc_regime in ("ranging", "neutral")

    if _was_blocked and _now_trending and _prev_btc_regime != "":
        # Unlocked: was choppy/neutral, now has a clear direction
        ...
        send_tg(
            ...
            f"BTC has shifted from <b>{_prev_btc_regime.upper()}</b> → "
            f"<b>{btc_regime.upper()}</b>.\n\n"
    ...
    elif _was_trending and _now_blocked:
        # Locked: was trending, now gone choppy/neutral
        if btc_regime == "ranging":
            regime_note = (
                "🤖 Bot is <b>STANDING ASIDE</b> — BTC EMAs are tightly coiled. "
                "No signals until a clear direction develops."
            )
        else:  # neutral
            regime_note = (
                "🤖 Bot is <b>scanning selectively</b> — will only fire on "
                "independent movers with confirmed structure and high conviction. "
                "Expect fewer signals until BTC breaks out."
            )
        send_tg(
            f"⚪ <b>BTC regime change: {btc_regime.upper()}</b>\n\n"
            f"BTC has shifted from <b>{_prev_btc_regime.upper()}</b> → "
            f"<b>{btc_regime.upper()}</b>.\n\n"
            f"{regime_note}",
            chat_id=TELEGRAM_CHAT_ID
        ...
        emoji = "🔄"
        send_tg(
            f"{emoji} <b>BTC regime flip: {_prev_btc_regime.upper()} → {btc_regime.upper()}</b>\n\n"
    ...
    # ── Neutral regime: scan continues but with a higher bar ─────────────
    # BTC neutral ≠ no opportunities. Independent tokens with confirmed
    # market structure still trend. We allow both directions but require:
    #   • Confirmed market structure (bullish or bearish, not neutral)
    #   • Low BTC correlation (< 0.55 — independent mover)
    #   • Score ≥ INTERNAL_MIN_SCORE + 10 (extra conviction required)
    neutral_mode = (btc_regime == "neutral")
    if neutral_mode:
        print(f"[Scanner] BTC is NEUTRAL — scanning for independent movers with confirmed structure.")
    ...
    for symbol, token_data in WATCHLIST.items():
        try:
            ...
            try:
                _fut = _tex.submit(analyse_token, symbol, token_data)
            ...
            # ── Neutral-regime extra gate ─────────────────────────────────
            # When BTC is neutral we only fire on tokens that are trending
            # independently. Requires all three:
            #   1. Confirmed market structure (token must have its own trend)
            #   2. Low BTC correlation (independent mover, corr < 0.55)
            #   3. Higher score floor (INTERNAL_MIN_SCORE + 10)
            if neutral_mode:
                token_struct = result.get("mkt_structure", "neutral")
                token_s4h    = result.get("mkt_structure_4h", "neutral")
                has_struct   = token_struct != "neutral" or token_s4h != "neutral"
                token_corr   = result.get("btc_corr")
                # 0.80 threshold: only blocks near-perfect BTC clones.
                # 0.55 is the counter-trend threshold; neutral BTC has no
                # direction, so a token at corr=0.60 can still have its own setup.
                is_indep     = token_corr is None or token_corr < 0.80
                neutral_score_ok = score >= INTERNAL_MIN_SCORE + 10

                if not has_struct:
                    print(f"  [{symbol}] Skipped (neutral BTC) — no confirmed token structure")
                    continue
                if not is_indep:
                    print(f"  [{symbol}] Skipped (neutral BTC) — pure BTC clone (corr={token_corr})")
                    continue
                if not neutral_score_ok:
                    print(f"  [{symbol}] Skipped (neutral BTC) — score {score} below neutral bar ({INTERNAL_MIN_SCORE + 10})")
                    continue
            ...
            if dire == "SHORT" and t1 == "uptrend":
                ...
                # elevated AND 1H MACD has turned bearish, momentum is
                # rolling over while price is still extended above the mean.
                # This is a high-probability mean-reversion SHORT setup.
                rsi_1h   = result["rsi"]
                m_bear   = result["macd_bearish"]
                ext_4h   = result.get("price_above_4h_e20_ext", False)
                overbought_ext = (rsi_1h > 68 and m_bear) or (rsi_1h > 72 and ext_4h)
                if not overbought_ext:
                    print(f"  [{symbol}] Skipped — 1H uptrend blocks SHORT "
                          f"(RSI={rsi_1h:.0f}, macd_bearish={m_bear}, ext_4h={ext_4h})")
                    continue
...
def main():
    ...
    print("  Filters: ADX · Donchian · DFT spectral · Market structure")
    print("           Overextension gate · Pullback entry · Vol threshold")
    print("           Orderbook imbalance · Manipulation detector")
    print("           BTC correlation · Onchain gate · Correlation cap")
    print("=" * 60)
